...
# POL-6b: the citation-bound constant _CITATION_LOG_MAX moved there too.
...
# (/pre-write-check) and main()'s can-write dispatch resolve unchanged.
...
# THE RE-EXPORT SURFACE. Every name above is imported to be reached THROUGH this file:
# writ/server/__init__.py loads it with spec_from_file_location and the routes reach
# `server.writ_session.<name>`, and several hooks load it by path with importlib. Nine
# comment blocks said so in prose, which neither a reader nor pyflakes can check; this says
# it in one place both can. `main` is defined here rather than re-exported, so it is not part
# of the surface. Nothing in this repository star-imports anything, so this has no runtime
# effect beyond stating the intent.
__all__ = [
    '_log_friction_event',
    'DEFAULT_SESSION_BUDGET',
    'APPROX_TOKENS_PER_RULE_FULL',

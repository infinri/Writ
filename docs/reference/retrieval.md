...
Regression floors (`tests/fixtures/regression_floors.py`; floors are the fail line, deliberately below measurement, never targets): MRR@5 >= 0.45 on the 47 ambiguous queries, hit rate >= 0.75 on all 193, domain-hit-rate@5 >= 0.90, nDCG@10 >= 0.65. Measured 2026-07-17: ~0.57 / ~0.78. The floor history records each downward walk as corpus-growth dilution with the measurements that justified it. Negative-query ground truth (20 off-domain and near-domain queries) pins the false-injection diagnostic behind the abstention gate. The methodology corpus has its own signed-off 40-query set (blocker floors 0.78 MRR, 0.90 hit rate).

...
- Final score = weighted sum with the semantic preset `w_bm25=0.198, w_vector=0.594, w_severity=0.099, w_confidence=0.099, w_graph=0.01` (literal preset: 0.396/0.396/0.099/0.099/0.01). Weights validate to sum 1.0. These are code constants in `ranking.py`, not configuration. A sixth weight, `w_bundle_cohesion`, was removed on 2026-08-06: no value improved all three metrics, and its intended home (methodology nodes ranked by this pipeline) never materialized because the methodology channel shipped deterministic. See `benchmarks/RANKING-LEVERS-2026-08-06.md`.
...
**Project scoping, by node TYPE and not by tag** (`writ/retrieval/node_scope.py::is_visible`, enforced at THREE points, each of which was a live bypass until it was closed: `_filter_candidates` for the candidates themselves, identically for keyword and vector hits; `_scope_enrichment` for the neighbour list attached to a surviving candidate, which reached the caller as the rendered `RELATED:` line; and `compute_graph_proximity` for the hop sets feeding the ranking score. The adjacency cache is deliberately built UNFILTERED, because one cache serves every caller, so scoping happens at each point of use. A neighbour's node type defaults to empty there rather than to `Rule` as the candidate filter does, because a neighbour can carry any label and defaulting to a doctrine type would fail open): `DOCTRINE_NODE_TYPES` (Rule, Skill, Playbook, Technique, AntiPattern, ForbiddenResponse) is universal and reaches every caller whatever tag it carries; everything else is a **record** (Memory, Decision, FileChange, Commit) and is limited to `{caller_project, "_shared"}`. A caller with no project resolves to no records at all, only `_shared` ones: `project=None` no longer disables the filter, and an unregistered cwd resolves to `""` rather than defaulting to `"writ"`. Tag scoping could not do this job: all 287 Rule nodes and every methodology node carry `project: "writ"`, so scoping doctrine by tag would have excluded the whole corpus from every project except this one while isolation tests still passed. The allowlist is written out, never derived, so an unanticipated node type is treated as a record and fails closed into a visible retrieval gap instead of a silent cross-project admission.

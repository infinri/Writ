...
- **`WRIT_CACHE_DIR`** defaults to a fresh `mkdtemp`: without it, subprocess tests would pollute the real session directory (the production default is the XDG state root, so without the pin every subprocess test would write into the operator's real state directory).

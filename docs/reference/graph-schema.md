...
- `GRAPH_ONLY_FIELDS` (`export.py:39`) are stripped on export and re-derived on ingest (`confidence`, `authority`, `times_seen_*`, `evidence`, `staleness_window`, `last_validated`, `source_origin`; `provenance` omitted only at its default). `mandatory` round-trips.
...
2. **Graduate**: at 50+ observations with >= 0.75 positive ratio (`writ/frequency.py`), `proposed` flips to `graduation_pending`, race-guarded, changing nothing else.
...
Interactive authoring (`writ add` / `writ edit`) pre-checks id collisions before MERGE would silently update, warns when a graph-only edge on a source-backed rule would be reconciled away, and `downweight` deliberately works at any authority (only promote/reject are provisional-gated).

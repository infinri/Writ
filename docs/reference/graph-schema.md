...
**SubagentRole fields** (`schema.py`): `role_id`, `name`, `prompt_template`, `dispatched_by`, `model_preference`, `tools`, `description`, and `write_scope`. `write_scope` is the paths a dispatched sub-agent running as that role may write, enforced by the write gate (see `session-and-gates.md`), and ABSENT AND EMPTY ARE DIFFERENT FACTS: no property at all (the model default is `None`, never `[]`) means the role declares no scope and the sub-agent keeps the blanket write allow, while `write_scope: []` means the role declares it writes nothing and every path is refused. Nothing on the read path coalesces one into the other, so a glob list authored in `bible/methodology/ROL-*.md` frontmatter arrives as a list and an unauthored one arrives as null. It is a MANAGED prop (a declared model field), which is what keeps `writ reconcile` and prop-parity syncing it instead of clearing it. 4 of the 5 roles declare one; `writ-implementer` deliberately does not, because its scope is the approved plan's `## Files` list, which is per-dispatch data rather than a property of the role. `writ doctor`'s `subagent-role-scope-coverage` reports the share.
...
- **Dual-location rules**: 14 ids (`_METHODOLOGY_CANONICAL_RULE_IDS`, `export.py`) export to `bible/methodology/<ID>.md` and are excluded from domain `rules.md`. Hand-editing them is safe; new graduated canon must be added to the list or export produces a duplicate.
...
**The dump.** `writ export-cypher` writes `writ-corpus.cypher` deterministically (staged `_dump_id`, sorted, stripped at the end); `writ import-cypher` wipes first because the CREATEs are not idempotent. 2026-08-14 dump: 468 nodes (Rule 288 of which 32 mandatory, Abstraction 62, Category 22, Phase 20, Skill 16, Playbook 15, AntiPattern 14, Technique 14, SubagentRole 5, Rationalization 4, WorkedExample 3, PressureScenario 3, ForbiddenResponse 2), 1,268 edges (RELATED_TO 512, BELONGS_TO 401, ABSTRACTS 186, DEMONSTRATES 29, COUNTERS 24, TEACHES 23, ...).

...
**Session identity**: `$CLAUDE_SESSION_ID` > `$CLAUDE_JOB_DIR` basename > the `/tmp/writ-current-session` pointer (rewritten every prompt) > newest cache by mtime. On a harness-rotated session id, the SessionStart hook carries the **mode only** into the fresh session, same project only; gates are never inherited.

**Compaction boundary**: PreCompact drops full rule objects (keeps ids); PostCompact is the authoritative recovery signal: budget resets to 8,000, the phase-scoped exclusion list clears, and the model is re-told its mode and phase plus a re-verify directive (pre-compaction verification output is second-hand).

## 2. Modes

`MODE_CONFIG` (`writ/session/mode_engine.py`) is the single source; five modes: `conversation`, `debug`, `investigate`, `review`, `work`.

- **`mode set`** always re-initializes: phase to the mode's initial phase, gates cleared, denial counts cleared, `project_root` stamped from cwd.
...
- Two confirmation sources, one endpoint: the pattern path (`auto-approve-gate.sh`, `confirmation_source="pattern"`, only in work mode during planning/testing) and the tool path (`/writ-approve`, `confirmation_source="tool"`, which must send `cwd` because the server cannot substitute its own working directory). Approval writes a `.claude/gates/<gate>.approved` artifact: an audit record, not the source of truth.

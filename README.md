...
A second thing is unproven, and smaller only by comparison: the graph traversal stage is the reason this project needs a graph database at all, and its individual contribution has never been isolated. The number will be published wherever it lands, including at or near zero, and the full reasoning is in [`docs/reference/retrieval.md`](docs/reference/retrieval.md).
